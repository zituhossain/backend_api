...
const {
	updateAllPlacesWithCommentTitleData,
} = require('./ReportController.js');
...
exports.fetchallPlaceCommentTitle = async (req, res) => {
	...
	if (allNgoDetails) {
		return apiResponse.successResponseWithData(
			res,
			'ngo_details_info fetch successfully.',
			allNgoDetails
		);
	} else {
...
exports.getPlaceCommentTitleById = async (req, res) => {
	try {
		...
		const details_data = await ngo_details_info.findOne({
			include: [Place],
...
exports.updatePlaceCommentTitle = async (req, res) => {
	try {
		...
		if (details_data) {
			if (req.body.title) {
				await ngo_details_info.update(req.body, {
					where: { id: ngo_details_id },
				});

				updateAllPlacesWithCommentTitleData(ngo_details_id);
