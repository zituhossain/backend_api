...
exports.AllPlaceHistory = async (req, res) => {
    ...
    try {
        ...
        const place_data = await year_place_ngo_officer.sequelize.query('SELECT GROUP_CONCAT(DISTINCT(ngo_id)) as ngoID,GROUP_CONCAT(DISTINCT(Ngos.name) ORDER BY Ngos.id ASC) as ngo_list,GROUP_CONCAT(DISTINCT(Ngos.short_name)) as ngo_short_name,GROUP_CONCAT(DISTINCT(Ngos.color_code) ORDER BY Ngos.id ASC) as color_list,years.name as year_id,years.bn_term as term,(SELECT GROUP_CONCAT(cnt) cnt FROM ( SELECT COUNT(*) cnt,year_id FROM year_place_ngo_officers ypno where rank=1 GROUP BY ypno.ngo_id,year_id )as totla WHERE totla.year_id = year_place_ngo_officers.year_id) as percent_list FROM `year_place_ngo_officers` LEFT join Ngos on Ngos.id = year_place_ngo_officers.ngo_id LEFT JOIN years on year_place_ngo_officers.year_id = years.id where rank=1 GROUP by year_id order by year_place_ngo_officers.year_id desc', { type: year_place_ngo_officer.sequelize.QueryTypes.SELECT });
...
exports.placeHistoryDivision = async (req, res) => {
    ...
    try {
        ...
        const [results, metadata] = await year_place_ngo_officer.sequelize.query(`SELECT GROUP_CONCAT(DISTINCT(year_place_ngo_officers.ngo_id)) as ngoID,GROUP_CONCAT(DISTINCT(Ngos.name) order by Ngos.id asc) as ngo_list,GROUP_CONCAT(DISTINCT(Ngos.short_name)) as ngo_short_name,GROUP_CONCAT(DISTINCT(Ngos.color_code) order by Ngos.id asc) as color_list,years.name as year_id,years.bn_term as term,(SELECT GROUP_CONCAT(cnt) cnt FROM 
