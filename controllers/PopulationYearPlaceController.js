...
exports.getbyDisId = async (req, res) => {
    ...
    try {
        ...
        const [results, metadata] = await population_year_place.sequelize.query('select sum(pyp.minority) as total_minority,sum(pyp.minority1) as total_minority1,sum(pyp.minority2) as total_minority2,sum(pyp.total_population) as total_population,sum(pyp.male) as total_male,sum(pyp.female) as total_female from population_year_places pyp left join places on places.id = pyp.place_id where places.district_id = '+disId+' and year_id = (select max(id) as year_id from years)');
...
exports.getbyPlaceId = async (req, res) => {
    ...
    try {
        ...
        const [results, metadata] = await population_year_place.sequelize.query('select sum(pyp.total_population) as total_population,sum(pyp.male) as total_male,sum(pyp.female) as total_female from population_year_places pyp left join places on places.id = pyp.place_id where places.id = '+placeId+' and year_id = (select max(id) as year_id from years)');
...
exports.getMinoritybyPlaceId = async (req, res) => {
    ...
    try {
        ...
        const [results, metadata] = await population_year_place.sequelize.query('select sum(pyp.minority) as total_minority,sum(pyp.minority1) as total_minority1,sum(pyp.minority2) as total_minority2 from population_year_places pyp left join places on places.id = pyp.place_id where places.id = '+placeId+' and year_id = (select max(id) as year_id from years)');
