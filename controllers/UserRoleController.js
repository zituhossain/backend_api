...
exports.copyUserRole = async (req, res) => {
    try {
      ...
      const newName = incrementLastCounter(role_data.name, maxId);
        ...
        const previlegeData = await Previlege_table.findAll({ where: { user_role_id: role_id } });
        ...
        if (previlegeData && previlegeData.length > 0) {
          ...
          return apiResponse.successResponse(res, 'Role copied and inserted successfully.');
        } else {
